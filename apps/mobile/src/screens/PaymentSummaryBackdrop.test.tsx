...
describe("PaymentSummaryBackdrop", () => {
  it("shows the masked card number, brand, cardholder name, and order total", () => {
    render(<PaymentSummaryBackdrop open card={card} items={items} isSubmitting={false} onCancel={jest.fn()} onPay={jest.fn()} />);
  ...
  it("shows a loading Pay button and blocks double submission while a payment is in flight", () => {
    const onPay = jest.fn();
    render(<PaymentSummaryBackdrop open card={card} items={items} isSubmitting onCancel={jest.fn()} onPay={onPay} />);
  ...
  it("calls onPay once when pressed while idle", () => {
    const onPay = jest.fn();
    render(<PaymentSummaryBackdrop open card={card} items={items} isSubmitting={false} onCancel={jest.fn()} onPay={onPay} />);
  ...
  it("shows a retryable error toast without losing the entered card or losing the total", () => {
    render(<PaymentSummaryBackdrop open card={card} items={items} isSubmitting={false} errorMessage="Payment was declined" onCancel={jest.fn()} onPay={jest.fn()} />);
