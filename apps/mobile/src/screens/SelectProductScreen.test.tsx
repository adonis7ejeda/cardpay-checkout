...
describe("SelectProductScreen", () => {
  it("lists selected items with a line subtotal and an order total", () => {
    render(<SelectProductScreen items={items} productNames={{ "sku-1": "Wireless Headphones" }} stockByProduct={{ "sku-1": 4 }} onChangeQuantity={jest.fn()} onRemove={jest.fn()} onContinue={jest.fn()} />);
  ...
  it("disables Continue to Checkout when the cart is empty and blocks navigation", () => {
    ...
    render(<SelectProductScreen items={[]} productNames={{}} stockByProduct={{}} onChangeQuantity={jest.fn()} onRemove={jest.fn()} onContinue={onContinue} />);
  ...
  it("removes an item instead of allowing its quantity to reach zero", () => {
    ...
    render(<SelectProductScreen items={items} productNames={{ "sku-1": "Wireless Headphones" }} stockByProduct={{ "sku-1": 4 }} onChangeQuantity={jest.fn()} onRemove={onRemove} onContinue={jest.fn()} />);
  ...
  it("never allows a stepper increase past available stock", () => {
    ...
    render(<SelectProductScreen items={items} productNames={{ "sku-1": "Wireless Headphones" }} stockByProduct={{ "sku-1": 2 }} onChangeQuantity={onChangeQuantity} onRemove={jest.fn()} onContinue={jest.fn()} />);
    expect(screen.getByRole("button", { name: "Increase quantity" })).toBeDisabled();
  });

  it("increases the quantity when stock allows it", () => {
    const onChangeQuantity = jest.fn();
    render(<SelectProductScreen items={items} productNames={{ "sku-1": "Wireless Headphones" }} stockByProduct={{ "sku-1": 4 }} onChangeQuantity={onChangeQuantity} onRemove={jest.fn()} onContinue={jest.fn()} />);
  ...
  it("disables the decrease stepper at quantity 1", () => {
    ...
    render(<SelectProductScreen items={singleItem} productNames={{ "sku-1": "Wireless Headphones" }} stockByProduct={{ "sku-1": 4 }} onChangeQuantity={jest.fn()} onRemove={jest.fn()} onContinue={jest.fn()} />);
